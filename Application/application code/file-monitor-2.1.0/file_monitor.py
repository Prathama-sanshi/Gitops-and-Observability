...
#  Setup structural logging to write into the shared volume
log_file_path = os.path.join(LOGGING_DIR, "app.log")
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s [%(levelname)s] %(message)s',
    handlers=[
        RotatingFileHandler(log_file_path, maxBytes=10*1024*1024, backupCount=5), # 10Mb and 5 backup files
        logging.StreamHandler() # Keeps logs visible in standard 'kubectl logs'
    ]
)
...
def update_system_metrics():
    ...
    logging.info(f"[System Metrics] CPU: {cpu_usage}%, Memory: {memory.percent}%, Disk: {disk.percent}%")
...
def update_metrics():
    ...
    logging.info(f"[File Metrics] log: {log_count}, txt: {txt_count}")
...
def file_generator():
    ...
    while True:
        ...
        for _ in range(num_files):
            ...
            logging.info(f"[Generator] Created: {filename}")
    ...
    while True:
        ...
        if all_files:
            ...
            for filepath in files_to_delete:
                try:
                    os.remove(filepath)
                    logging.info(f"[Deleter] Deleted: {os.path.basename(filepath)}")
                except (PermissionError, OSError) as e:
                    logging.error(f"[Deleter] Error deleting {os.path.basename(filepath)}: {e}")
...
def main():
    logging.info("Starting File Monitor Application...")
    logging.info(f"Monitoring directory: {os.path.abspath(LOGGING_DIR)}")
    logging.info(f"Metrics endpoint: http://localhost:{METRICS_PORT}/metrics")
